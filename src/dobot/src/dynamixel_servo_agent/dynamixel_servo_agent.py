...
# new current sensor 1st Mar 2019 INA219
...
# Global params for current sensor
#i2c_bus = busio.I2C(SCL, SDA)
#ina219 = adafruit_ina219.INA219(i2c_bus)

cwd = os.path.dirname(os.path.realpath(__file__))
print cwd
proc = subprocess.Popen(['python3',cwd + '/ina219.py'],stdout=subprocess.PIPE)
#proc = subprocess.Popen(['python3','/home/ubuntu/dobot_conveyor_unit_polishing/CONVEYOR_UNIT/src/dobot/src/dynamixel_servo_agent/ina219.py'],stdout=subprocess.PIPE)
...
def refresh_motor_readings() :
    global line_availability, old_current, current, old_velocity, velocity, old_temperature, temperature, old_pwm, pwm, old_supply_voltage, supply_voltage
    global proc
    ...
    old_current = current
    #current = round((acs_offset-adc.read_adc(1,gain=1))*5.405/8,2) # 185 mV per$
    try:
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        data.raw_current = current / 1000.0
