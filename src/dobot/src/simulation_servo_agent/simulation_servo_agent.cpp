...
//  C++ source code generated on : Sun Jan 20 16:11:54 2019
//
...
static void simulation_servo_a_RandSrc_GZ_D(real_T y[], const real_T mean[],
  ...
  int32_T nChans, int32_T nSamps)
{
  ...
  for (chan = 0; chan < nChans; chan++) {
    ...
    for (samp = 0; samp < nSamps; samp++) {
      ...
      jsr ^= jsr >> 17;
      jsr ^= jsr << 5;
      simulation_servo_agent_B.i = (int32_T)(icng + jsr);
      j = (simulation_servo_agent_B.i & 63) + 1;
      simulation_servo_agent_B.r = (real_T)simulation_servo_agent_B.i *
        4.6566128730773926E-10 * vt[j];
      if (!(fabs(simulation_servo_agent_B.r) <= vt[j - 1])) {
        simulation_servo_agent_B.x = (fabs(simulation_servo_agent_B.r) - vt[j -
          1]) / (vt[j] - vt[j - 1]);
        icng = 69069U * icng + 1234567U;
        jsr ^= jsr << 13;
        jsr ^= jsr >> 17;
        jsr ^= jsr << 5;
        simulation_servo_agent_B.y = (real_T)(int32_T)(icng + jsr) *
          2.328306436538696E-10 + 0.5;
        simulation_servo_agent_B.s = simulation_servo_agent_B.x +
          simulation_servo_agent_B.y;
        if (simulation_servo_agent_B.s > 1.301198) {
          simulation_servo_agent_B.r = simulation_servo_agent_B.r < 0.0 ?
            0.4878992 * simulation_servo_agent_B.x - 0.4878992 : 0.4878992 -
            0.4878992 * simulation_servo_agent_B.x;
        } else {
          if (!(simulation_servo_agent_B.s <= 0.9689279)) {
            simulation_servo_agent_B.x = 0.4878992 - 0.4878992 *
              simulation_servo_agent_B.x;
            if (simulation_servo_agent_B.y > 12.67706 - exp(-0.5 *
                 simulation_servo_agent_B.x * simulation_servo_agent_B.x) *
                12.37586) {
              simulation_servo_agent_B.r = simulation_servo_agent_B.r < 0.0 ?
                -simulation_servo_agent_B.x : simulation_servo_agent_B.x;
            } else {
              if (!(exp(-0.5 * vt[j] * vt[j]) + simulation_servo_agent_B.y *
                    0.01958303 / vt[j] <= exp(-0.5 * simulation_servo_agent_B.r *
                    simulation_servo_agent_B.r))) {
                do {
                  icng = 69069U * icng + 1234567U;
                  jsr ^= jsr << 13;
                  jsr ^= jsr >> 17;
                  jsr ^= jsr << 5;
                  simulation_servo_agent_B.x = log((real_T)(int32_T)(icng + jsr)
                ...
                simulation_servo_agent_B.r = simulation_servo_agent_B.r < 0.0 ?
                  simulation_servo_agent_B.x - 2.776994 : 2.776994 -
                  simulation_servo_agent_B.x;
              }
            }
          }
      ...
      y[chan * nSamps + samp] = xstd[xstdLen > 1 ? chan : 0] *
        simulation_servo_agent_B.r + mean[meanLen > 1 ? chan : 0];
    }
  ...
  // End of S-Function (sdsprandsrc2): '<S20>/Random Source'
}
  ...
  dsp_MovingAverage_simulation__T *obj, real_T u)
{
  ...
  simulation_servo_agent_B.cumRevIndex_n = obj_0->pCumRevIndex;
  simulation_servo_agent_B.csum_p = obj_0->pCumSum;
  for (i = 0; i < 999; i++) {
    simulation_servo_agent_B.csumrev[i] = obj_0->pCumSumRev[i];
  }

  simulation_servo_agent_B.csum_p += u;
  i = (int32_T)simulation_servo_agent_B.cumRevIndex_n - 1;
  z = simulation_servo_agent_B.csumrev[i] + simulation_servo_agent_B.csum_p;
  simulation_servo_agent_B.csumrev[i] = u;
  if (simulation_servo_agent_B.cumRevIndex_n != 999.0) {
    simulation_servo_agent_B.cumRevIndex_n++;
  } else {
    simulation_servo_agent_B.cumRevIndex_n = 1.0;
    simulation_servo_agent_B.csum_p = 0.0;
    for (i = 0; i < 998; i++) {
  ...
  y = z / 1000.0;
  obj_0->pCumSum = simulation_servo_agent_B.csum_p;
  for (i = 0; i < 999; i++) {
    obj_0->pCumSumRev[i] = simulation_servo_agent_B.csumrev[i];
  }

  obj_0->pCumRevIndex = simulation_servo_agent_B.cumRevIndex_n;
...
void RandSrcInitState_GZ(const uint32_T seed[], uint32_T state[], int32_T nChans)
{
  ...
  // InitializeConditions for S-Function (sdsprandsrc2): '<S20>/Random Source'
  // RandSrcInitState_GZ
  for (i = 0; i < nChans; i++) {
  ...
  // End of InitializeConditions for S-Function (sdsprandsrc2): '<S20>/Random Source' 
}
...
{
  {
    ...
    // Start for MATLABSystem: '<S37>/SourceBlock' incorporates:
    //   Inport: '<S38>/In1'

    p = Sub_simulation_servo_agent_3760.getLatestMessage
      (&simulation_servo_agent_B.b_varargout_2);

    // Outputs for Enabled SubSystem: '<S37>/Enabled Subsystem' incorporates:
    //   EnablePort: '<S38>/Enable'

    if (p) {
      simulation_servo_agent_B.In1 = simulation_servo_agent_B.b_varargout_2;
    }
    ...
    if (simulation_servo_agent_DW.LoopSubsystem_MODE) {
      ...
      // SimscapeInputBlock: '<S35>/INPUT_4_1_1'
      simulation_servo_agent_B.INPUT_4_1_1[0] =
        simulation_servo_agent_B.DataTypeConversion2;
      simulation_servo_agent_B.INPUT_4_1_1[1] = 0.0;
      simulation_servo_agent_B.INPUT_4_1_1[2] = 0.0;
      simulation_servo_agent_DW.INPUT_4_1_1_discrete[0] =
        !(simulation_servo_agent_B.INPUT_4_1_1[0] ==
          simulation_servo_agent_DW.INPUT_4_1_1_discrete[1]);
      simulation_servo_agent_DW.INPUT_4_1_1_discrete[1] =
        simulation_servo_agent_B.INPUT_4_1_1[0];
      simulation_servo_agent_B.INPUT_4_1_1[0] =
        simulation_servo_agent_DW.INPUT_4_1_1_discrete[1];
      simulation_servo_agent_B.INPUT_4_1_1[3] =
        simulation_servo_agent_DW.INPUT_4_1_1_discrete[0];

      // Abs: '<S3>/Abs' incorporates:
      //   UnitDelay: '<S3>/Unit Delay2'

      simulation_servo_agent_B.Abs = fabs
        (simulation_servo_agent_DW.UnitDelay2_DSTATE);

      // Switch: '<S3>/Switch' incorporates:
      //   Fcn: '<S3>/f(x)'
      //   Fcn: '<S3>/g(x)'

      if (simulation_servo_agent_B.Abs >
          simulation_servo_agent_P.Switch_Threshold) {
        simulation_servo_agent_B.Switch = (0.14285714285714285 *
          simulation_servo_agent_B.Abs + 10.0) * 2.0e-9;
      } else {
        simulation_servo_agent_B.Switch = 2.0E-8 * simulation_servo_agent_B.Abs;
      }

      // End of Switch: '<S3>/Switch'

      // SimscapeInputBlock: '<S35>/INPUT_3_1_1'
      simulation_servo_agent_B.INPUT_3_1_1[0] = simulation_servo_agent_B.Switch;
      ...
      simulationData->mData->mModeVector.mX =
        &simulation_servo_agent_DW.STATE_1_Modes;
      p = false;
      ...
      simulation_servo_agent_B.dv3[8] = simulation_servo_agent_B.INPUT_4_1_1[0];
      simulation_servo_agent_B.dv3[9] = simulation_servo_agent_B.INPUT_4_1_1[1];
      simulation_servo_agent_B.dv3[10] = simulation_servo_agent_B.INPUT_4_1_1[2];
      simulation_servo_agent_B.dv3[11] = simulation_servo_agent_B.INPUT_4_1_1[3];
      simulation_servo_agent_B.iv3[3] = 12;
      simulation_servo_agent_B.dv3[12] = simulation_servo_agent_B.INPUT_3_1_1[0];
      simulation_servo_agent_B.dv3[13] = simulation_servo_agent_B.INPUT_3_1_1[1];
      simulation_servo_agent_B.dv3[14] = simulation_servo_agent_B.INPUT_3_1_1[2];
      simulation_servo_agent_B.dv3[15] = simulation_servo_agent_B.INPUT_3_1_1[3];
      simulation_servo_agent_B.iv3[4] = 16;
      ...
      simulation_servo_agent_B.z_tmp = ne_simulator_method((NeslSimulator *)
        simulation_servo_agent_DW.STATE_1_Simulator, NESL_SIM_OUTPUTS,
        simulationData, diagnosticManager);
      if (simulation_servo_agent_B.z_tmp != 0) {
      ...
      simulation_servo_agent_B.dv0[8] = simulation_servo_agent_B.INPUT_4_1_1[0];
      simulation_servo_agent_B.dv0[9] = simulation_servo_agent_B.INPUT_4_1_1[1];
      simulation_servo_agent_B.dv0[10] = simulation_servo_agent_B.INPUT_4_1_1[2];
      simulation_servo_agent_B.dv0[11] = simulation_servo_agent_B.INPUT_4_1_1[3];
      simulation_servo_agent_B.iv0[3] = 12;
      simulation_servo_agent_B.dv0[12] = simulation_servo_agent_B.INPUT_3_1_1[0];
      simulation_servo_agent_B.dv0[13] = simulation_servo_agent_B.INPUT_3_1_1[1];
      simulation_servo_agent_B.dv0[14] = simulation_servo_agent_B.INPUT_3_1_1[2];
      simulation_servo_agent_B.dv0[15] = simulation_servo_agent_B.INPUT_3_1_1[3];
      simulation_servo_agent_B.iv0[4] = 16;
      memcpy(&simulation_servo_agent_B.dv0[16],
             &simulation_servo_agent_B.STATE_1[0], 12U * sizeof(real_T));
      simulation_servo_agent_B.iv0[5] = 28;
      ...
      // Gain: '<S29>/GAIN'
      simulation_servo_agent_B.Abs = simulation_servo_agent_P.GAIN_Gain *
        simulation_servo_agent_B.OUTPUT_1_0[2];

      // Rounding: '<S10>/Round' incorporates:
      //   Constant: '<S10>/Constant3'
      //   Product: '<S10>/Divide'

      simulation_servo_agent_B.Abs = rt_roundd_snf(simulation_servo_agent_B.Abs /
      ...
      simulation_servo_agent_DW.UnitDelay2_DSTATE = simulation_servo_agent_B.Abs
        * simulation_servo_agent_P.Constant4_Value;
      ...
      for (simulation_servo_agent_B.z_tmp = 0; simulation_servo_agent_B.z_tmp <
           299; simulation_servo_agent_B.z_tmp++) {
        simulation_servo_agent_B.csumrev_m[simulation_servo_agent_B.z_tmp] =
          simulation_servo_agent_DW.obj_i.pStatistic->
          pCumSumRev[simulation_servo_agent_B.z_tmp];
      }

      simulation_servo_agent_B.csum +=
        simulation_servo_agent_DW.UnitDelay2_DSTATE;
      simulation_servo_agent_B.z_tmp = (int32_T)
        simulation_servo_agent_B.cumRevIndex - 1;
      simulation_servo_agent_B.z =
        simulation_servo_agent_B.csumrev_m[simulation_servo_agent_B.z_tmp] +
        simulation_servo_agent_B.csum;
      simulation_servo_agent_B.csumrev_m[simulation_servo_agent_B.z_tmp] =
        simulation_servo_agent_DW.UnitDelay2_DSTATE;
      if (simulation_servo_agent_B.cumRevIndex != 299.0) {
        ...
        for (simulation_servo_agent_B.z_tmp = 0; simulation_servo_agent_B.z_tmp <
             298; simulation_servo_agent_B.z_tmp++) {
          simulation_servo_agent_B.csumrev_m[297 -
            simulation_servo_agent_B.z_tmp] +=
            simulation_servo_agent_B.csumrev_m[298 -
            simulation_servo_agent_B.z_tmp];
        }
      }
      ...
      simulation_servo_agent_B.dv1[8] = simulation_servo_agent_B.INPUT_4_1_1[0];
      simulation_servo_agent_B.dv1[9] = simulation_servo_agent_B.INPUT_4_1_1[1];
      simulation_servo_agent_B.dv1[10] = simulation_servo_agent_B.INPUT_4_1_1[2];
      simulation_servo_agent_B.dv1[11] = simulation_servo_agent_B.INPUT_4_1_1[3];
      simulation_servo_agent_B.iv1[3] = 12;
      simulation_servo_agent_B.dv1[12] = simulation_servo_agent_B.INPUT_3_1_1[0];
      simulation_servo_agent_B.dv1[13] = simulation_servo_agent_B.INPUT_3_1_1[1];
      simulation_servo_agent_B.dv1[14] = simulation_servo_agent_B.INPUT_3_1_1[2];
      simulation_servo_agent_B.dv1[15] = simulation_servo_agent_B.INPUT_3_1_1[3];
      simulation_servo_agent_B.iv1[4] = 16;
      memcpy(&simulation_servo_agent_B.dv1[16],
             &simulation_servo_agent_B.STATE_1[0], 12U * sizeof(real_T));
      simulation_servo_agent_B.iv1[5] = 28;
      simulationData->mData->mInputValues.mN = 28;
      ...
      // S-Function (sdsprandsrc2): '<S20>/Random Source'
      simulation_servo_a_RandSrc_GZ_D(&simulation_servo_agent_B.Abs,
        &simulation_servo_agent_P.RandomSource_MeanVal, 1,
        &simulation_servo_agent_P.RandomSource_VarianceRTP, 1,
        simulation_servo_agent_DW.RandomSource_STATE_DWORK, 1, 1);

      // Fcn: '<S7>/Fcn' incorporates:
      //   UnitDelay: '<S7>/Unit Delay2'

      simulation_servo_agent_DW.UnitDelay2_DSTATE_m = rt_powd_snf(((0.5958 *
        simulation_servo_agent_DW.UnitDelay2_DSTATE_m + 4.784) - 0.02393 *
        rt_powd_snf(simulation_servo_agent_DW.UnitDelay2_DSTATE_m, 2.0)) +
        0.0005953 * rt_powd_snf(simulation_servo_agent_DW.UnitDelay2_DSTATE_m,
        3.0), 2.0) / 18.0 * 1.0e-6;

      // S-Function (scomawgnchan2): '<S20>/Dynamic AWGN' incorporates:
      //   UnitDelay: '<S7>/Unit Delay2'

      simulation_servo_agent_DW.STDDEV = 0.0;
      simulation_servo_agent_B.DynamicAWGN = sqrt
        (simulation_servo_agent_DW.UnitDelay2_DSTATE_m) *
        simulation_servo_agent_B.Abs;
      simulation_servo_agent_B.DynamicAWGN +=
        simulation_servo_agent_B.OUTPUT_1_0[1];

      // Constant: '<S9>/Constant3'
      simulation_servo_agent_B.Constant3 =
        simulation_servo_agent_P.Constant3_Value_j;
      ...
      simulation_servo_agent_B.dv2[8] = simulation_servo_agent_B.INPUT_4_1_1[0];
      simulation_servo_agent_B.dv2[9] = simulation_servo_agent_B.INPUT_4_1_1[1];
      simulation_servo_agent_B.dv2[10] = simulation_servo_agent_B.INPUT_4_1_1[2];
      simulation_servo_agent_B.dv2[11] = simulation_servo_agent_B.INPUT_4_1_1[3];
      simulation_servo_agent_B.iv2[3] = 12;
      simulation_servo_agent_B.dv2[12] = simulation_servo_agent_B.INPUT_3_1_1[0];
      simulation_servo_agent_B.dv2[13] = simulation_servo_agent_B.INPUT_3_1_1[1];
      simulation_servo_agent_B.dv2[14] = simulation_servo_agent_B.INPUT_3_1_1[2];
      simulation_servo_agent_B.dv2[15] = simulation_servo_agent_B.INPUT_3_1_1[3];
      simulation_servo_agent_B.iv2[4] = 16;
      memcpy(&simulation_servo_agent_B.dv2[16],
             &simulation_servo_agent_B.STATE_1[0], 12U * sizeof(real_T));
      simulation_servo_agent_B.iv2[5] = 28;
      simulationData->mData->mInputValues.mN = 28;
    ...
    if (simulation_servo_agent_M->Timing.t[0] < 0.001) {
      ...
      // Start for MATLABSystem: '<S36>/Set Parameter' incorporates:
      //   Constant: '<S36>/Constant'

      ParamSet_simulation_servo_agent_3446.set_parameter
        (simulation_servo_agent_P.Constant_Value_b);

      // Start for MATLABSystem: '<S36>/Set Parameter1' incorporates:
      //   Constant: '<S36>/Constant'
...

  {
    ...
    if (simulation_servo_agent_DW.LoopSubsystem_MODE) {
      ...
      simulation_servo_agent_B.dv4[8] = simulation_servo_agent_B.INPUT_4_1_1[0];
      simulation_servo_agent_B.dv4[9] = simulation_servo_agent_B.INPUT_4_1_1[1];
      simulation_servo_agent_B.dv4[10] = simulation_servo_agent_B.INPUT_4_1_1[2];
      simulation_servo_agent_B.dv4[11] = simulation_servo_agent_B.INPUT_4_1_1[3];
      simulation_servo_agent_B.iv4[3] = 12;
      simulation_servo_agent_B.dv4[12] = simulation_servo_agent_B.INPUT_3_1_1[0];
      simulation_servo_agent_B.dv4[13] = simulation_servo_agent_B.INPUT_3_1_1[1];
      simulation_servo_agent_B.dv4[14] = simulation_servo_agent_B.INPUT_3_1_1[2];
      simulation_servo_agent_B.dv4[15] = simulation_servo_agent_B.INPUT_3_1_1[3];
      simulation_servo_agent_B.iv4[4] = 16;
      simulationData->mData->mInputValues.mN = 16;
...
void simulation_servo_agent_initialize(void)
{
...

  {
    ...
    simulationData = (NeslSimulationData *)
      simulation_servo_agent_DW.STATE_1_SimulationData;
    simulation_servo_agent_B.time_f = simulation_servo_agent_M->Timing.t[0];
    simulationData->mData->mTime.mN = 1;
    simulationData->mData->mTime.mX = &simulation_servo_agent_B.time_f;
    simulationData->mData->mContStates.mN = 0;
    simulationData->mData->mContStates.mX = NULL;
    simulationData->mData->mDiscStates.mN = 11;
    simulationData->mData->mDiscStates.mX = (real_T *)
    ...
    simulationData = (NeslSimulationData *)
      simulation_servo_agent_DW.OUTPUT_1_0_SimulationData;
    simulation_servo_agent_B.time_g = simulation_servo_agent_M->Timing.t[0];
    simulationData->mData->mTime.mN = 1;
    simulationData->mData->mTime.mX = &simulation_servo_agent_B.time_g;
    simulationData->mData->mContStates.mN = 0;
    simulationData->mData->mContStates.mX = NULL;
    simulationData->mData->mDiscStates.mN = 0;
    simulationData->mData->mDiscStates.mX = NULL;
    ...
    simulationData = (NeslSimulationData *)
      simulation_servo_agent_DW.OUTPUT_1_1_SimulationData;
    simulation_servo_agent_B.time_g1 = simulation_servo_agent_M->Timing.t[0];
    simulationData->mData->mTime.mN = 1;
    simulationData->mData->mTime.mX = &simulation_servo_agent_B.time_g1;
    simulationData->mData->mContStates.mN = 0;
    simulationData->mData->mContStates.mX = NULL;
    simulationData->mData->mDiscStates.mN = 0;
    simulationData->mData->mDiscStates.mX = NULL;
    ...
    simulation_servo_agent_DW.UnitDelay2_DSTATE =
      simulation_servo_agent_P.UnitDelay2_InitialCondition;
    ...
    // InitializeConditions for S-Function (sdsprandsrc2): '<S20>/Random Source' 
    simulation_servo_agent_DW.RandomSource_SEED_DWORK =
      simulation_servo_agent_P.AWGNChannel_seed;
    RandSrcInitState_GZ(&simulation_servo_agent_DW.RandomSource_SEED_DWORK,
                        simulation_servo_agent_DW.RandomSource_STATE_DWORK, 1);

    // InitializeConditions for UnitDelay: '<S7>/Unit Delay2'
    simulation_servo_agent_DW.UnitDelay2_DSTATE_m =
      simulation_servo_agent_P.UnitDelay2_InitialCondition_f;
...
void simulation_servo_agent_terminate(void)
{
  ...
  // End of Start for MATLABSystem: '<S3>/Moving Average1'
