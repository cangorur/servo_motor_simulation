...
/*     printf("bufSize = %d\n", bufSize); */
    ...
    if(initValueSize > bufSize) {       
       ...
       #if defined(MATLAB_MEX_FILE)
       mexWarnMsgIdAndTxt("SimscapePowerSystemsST:DiscreteVaraiableTimeDelay:InvalidParameter", "Block %s: Delay buffer has been increased to %d samples, in order to respect the initial condition length.", ssGetPath(S), initValueSize);       
       #endif
    }
...
static void mdlTerminate(SimStruct *S) 
{
    ...
    if (pinfo->delaySmallerThanTsFLAG == 1) {   
        #if defined(MATLAB_MEX_FILE)
        mexWarnMsgIdAndTxt("SimscapePowerSystemsST:DiscreteVaraiableTimeDelay:InvalidParameter", "Block %s: The specified delay time is smaller than the specified Sample time (Ts). This may cause inaccurate simulation results.", ssGetPath(S));                     
        #endif
    }
    /* If delayGreaterThanMaxDelayFLAG is 1, throw a warning */
    if (pinfo->delayGreaterThanMaxDelayFLAG == 1) {
        #if defined(MATLAB_MEX_FILE)
            mexWarnMsgIdAndTxt("SimscapePowerSystemsST:DiscreteVaraiableTimeDelay:InvalidParameter", "Block %s: The delay used is greater than the specified maximum delay parameter (%g). Limiting delay value to maximum delay.", ssGetPath(S), maxDelay);             
        #endif
    }
    /* If negativeOrInvalidDelayFLAG is 1, throw a warning */
    if (pinfo->negativeOrInvalidDelayFLAG == 1) {
        #if defined(MATLAB_MEX_FILE)
            mexWarnMsgIdAndTxt("SimscapePowerSystemsST:DiscreteVaraiableTimeDelay:InvalidParameter", "Block %s: Negative/NaN time delay is not allowed. Delay has been set to zero.", ssGetPath(S));                         
        #endif
    }
